...
export class CartService {
  ...
  public getCartPrice(cartItems: CartItemModel[]) {
    this.cartPrice = cartItems.reduce((acc, { businessService: { price } }) => {
      return acc + price;
    }, 0);
  }
  ...
  public removeServiceToCart(businessServiceId: number) {
    const cartItemId = this.cartItems.find(
      ({ businessService: { id } }) => id === businessServiceId,
    )?.id;
  ...
  public fetchAddress() {
    this.addressSerive.getAllAddress().subscribe({
      next: (res: any) => {
