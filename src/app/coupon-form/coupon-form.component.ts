...
export class CouponFormComponent implements OnInit {
  ...
  constructor(
    ...
    private readonly conf: DynamicDialogConfig,
  ) {}
  ngOnInit(): void {
    ...
    this.couponForm = new FormGroup({
      ...
      dateRange: new FormControl(
        this.createDataRange(coupon?.startTime, coupon?.endTime) ?? [],
        [Validators.required],
      ),
      percent: new FormControl(coupon?.percent ?? '', [
        Validators.required,
        Validators.min(1),
        Validators.max(99),
      ]),
      minimumAmount: new FormControl(coupon?.minimumAmount ?? '', [
        Validators.required,
      ]),
    });
  }
